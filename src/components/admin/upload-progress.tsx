...
export function UploadProgress({ state }: { state?: UploadState }) {
  ...
  return (
    <div className="mt-2 w-full max-w-sm" role="status" aria-live="polite">
      <div className="h-1.5 overflow-hidden rounded-full bg-secondary">
        <div className={`h-full rounded-full transition-[width] duration-300 ${color}`} style={{ width: `${state.percent}%` }} />
      </div>
